...
cams_switch = {
    ...
    "Side": "rtsp://192.168.1.123:8554/video6_unicast",
}
IPS = {
    "Switch": cams_switch["Tilt"],
    "Main": "rtsp://192.168.1.120:8554/video0_unicast",
    "Gripper_L": "rtsp://192.168.1.123:8554/video0_unicast",
    "Gripper_R": "rtsp://192.168.1.123:8554/video2_unicast",
    "Bottom": "rtsp://192.168.1.123:8554/video4_unicast",
}

POS = [[0, 0], [2 / 5, 0], [0, 1 / 2], [1 / 3, 1 / 2], [2 / 3, 1 / 2]]
...
class CoPilot:
    def __init__(self, window, geometry):
        ...
        self.dir = "/home/k3rnelpan1c/robotics/colcon_workspace/src/gui_pkg/gui_pkg/"
    ...
    def create_widgets(self):
        ...
        self.roll_label = tk.Label(
            self.angles_frame, text=self.roll, font=label_font, bg="#ffffff"
        )
        ...
        self.pitch_label = tk.Label(
            self.angles_frame, text=self.pitch, font=label_font, bg="#ffffff"
        )
        ...
        self.yaw_label = tk.Label(
            self.angles_frame, text=self.yaw, font=label_font, bg="#ffffff"
        )
        ...
        self.vx_label = tk.Label(
            self.angles_frame, text=self.vx, font=label_font, bg="#ffffff"
        )
        ...
        self.wz_label = tk.Label(
            self.angles_frame, text=self.wz, font=label_font, bg="#ffffff"
        )
    ...
    def switch_cam(self):
        ...
        if not SWITCH:
            ...
            IPS["Switch"] = cams_switch["Side"]

        PROCESSES[0].terminate()
        process = Process(
            target=displayCamera,
            args=(IPS["Switch"], "Switch", SIZE[0], POS[0], getScreensInfo(2)),
        )
        process.start()
...
class CoPilotNode(Node):
    ...
    def depth_callback(self, depth_msg: Float64):
        self.copilot.depth = depth_msg.data
        self.copilot_obj.depth_label.config(text=str(self.copilot.depth))

    def temp_callback(self, temp_msg):
        self.copilot.temperature = temp_msg.data
        self.copilot_obj.temperature_label.config(text=str(temperature))

    def vel_callback(self, twist_msg: Twist):

        self.copilot.vx = twist_msg.data.linear.x
        self.copilot.wz = twist_msg.data.angular.z
        self.copilot_obj.vx_label.config(text=str(vx))
        self.copilot_obj.wz_label.config(text=str(wz))
    ...
    def imu_callback(self, imu_msg):
        ...
        (self.roll, self.pitch, self.yaw) =     (orientation_list)
        self.yaw *= -1
        self.pitch *= -1
...
def main():
    ...
    # copilot_process = Process(target=init_copilot)
    # copilot_process.start()
    # pilot_process = Process(target=init_pilot, args=(getScreensInfo(2),IPS))
    # pilot_process.start()
