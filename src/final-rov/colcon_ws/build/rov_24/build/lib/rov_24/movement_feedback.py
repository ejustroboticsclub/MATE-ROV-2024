...
class Param:
    ...
    thruster_max = 1720.0
    thruster_min = 1250.0  # Changed to 1180 instead of 1160 to make sure that the center is 1485 which is a stoping value

    thruster_side_max = 1750.0
    thruster_side_min = 1240.0
...
class CalibrationNode(Node):
    def __init__(self):
        ...
        for i in CTRL:
            self.parameters_dict[i] = self.get_parameter(i).get_parameter_value().bool_value
    ...
    def timer_callback(self):
        ...
        if (self.get_parameter("depth_control").get_parameter_value().bool_value):
        ...
        if (self.get_parameter("imu_control").get_parameter_value().bool_value):
            stabilization_actuation = self.pid_stabilization.compute(self.desired.roll, self.actual.roll)
        ...
        thrusters_voltages.data = [int(phi_1), int(phi_2), int(phi_3), int(phi_4), int(phi_5), int(phi_6)]
        # publish the message
        self.get_logger().info(str(list(thrusters_voltages.data)))
        self.get_logger().info(f"actual depth: {self.actual.depth},\ndesired depth: {self.desired.depth}")
        self.get_logger().info(
    ...
    def imu_recieved_callback(self, imu: Imu):
        ...
        roll, _pith, yaw = euler_from_quaternion((imu.orientation.x, imu.orientation.y, imu.orientation.z, imu.orientation.w))
